...
"""
AlexNet
See original model at "ImageNet Classification with Deep Convolutional Neural Networks"
"""
import mxnet as mx
import numpy as np
...
def alexnet(config):
    # Parse config
    num_classes = abs(int(config['num_classes']))
    dtype = 'float32'
    if config.get('dtype'):
        dtype = str(config['dtype'])

    # Construct symbol(copy from mxnet)
